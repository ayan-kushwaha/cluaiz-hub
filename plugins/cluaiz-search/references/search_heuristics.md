...
This document contains advanced heuristics for the `cluaiz-search` plugin. When the engine is operating in complex analysis modes, refer to these strategies to maximize search quality.

## Provider Differences

The `cluaiz-search` plugin dynamically routes your queries to different backend providers based on the `manifest-plugin.yaml` configuration. You must optimize your query based on the active provider.
...
```cel
let $site_data = use plugin::cluaiz-search -> fetch(
...
```cel
let $results = use plugin::cluaiz-search -> query(
    q: "Rust memory model",
